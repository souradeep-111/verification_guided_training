...
void simulate_system_and_create_plots(
  int no_of_steps,
  network_handler system_network,
  unsigned int acceleration_number,
  vector< unsigned int > important_outputs,
...
)
{
    ...
    int i, j, k;
    int dim = 0;
    // Just a stupid loop to figure out the dimension
    i = 0;
    ...
    vector_of_weight_collection all_weights(system_network.no_of_outputs);
    vector_of_bias_collection all_biases(system_network.no_of_outputs);

    vector< vector< unsigned int > > active_weights;


    plotting_data system_plots(dim);
    ...
    while(!reach_sets.empty())
    {
      ...
      simulate_accelerated(system_network, acceleration_number, important_outputs, scaling_factor,
      ...
      scale_vector(biases_next, scaling_factor);
      ...
      next_set.time_stamp = current_set.time_stamp + acceleration_number;
    ...
    i = 1;
    while(i < acceleration_number)
    {


      simulate_accelerated(system_network, i, important_outputs, scaling_factor,
        offset_already, starting_set.region_constr, biases_next);
      adjust_offset(biases_next, offset_already);
      scale_vector(biases_next, scaling_factor);
      convert_direction_biases_to_constraints(total_directions,
    ...
    vector< datatype > system_point(dim);
    vector< double > x_next_point(dim);

    weight_collection weights_buffer;
    bias_collection bias_buffer;
    for(auto q = 0; q < system_network.no_of_outputs; q++)
    {
      if(important_outputs[q])
      {
        system_network.return_network_information(weights_buffer, bias_buffer);
        system_network.cast_to_single_output_network(weights_buffer, bias_buffer, q+1);
        all_weights[q] = weights_buffer;
        all_biases[q] = bias_buffer;
      }
    }
    ...
    int steps = system_plots.reach_set_time_range;
    ...
    while(k < no_of_plots)
    {
      ...
      return_random_sample_with_seed(starting_set.region_constr, initial_point, k * 23 );


      vector< double > x_point = initial_point;
      trace_data.clear();
      trace_data.push_back(x_point);
      ...
      while(i < (steps /* * acc_in_reach_sets*/))
      {
        ...
        x_next_point.clear();
        for(auto index = 0; index < system_network.no_of_outputs; index++)
        {

          if(important_outputs[index])  // THIS PART IS A BIT BRITTLE, BE CAREFUL. YOU ARE ASSUMING AN ORDERING
          {
            buffer = compute_network_output(x_point, all_weights[index], all_biases[index], active_weights);
            buffer += offset_already[index];
            buffer *= scaling_factor[index];

            x_next_point.push_back(buffer);

          }
        }


        x_point = x_next_point;
        trace_data.push_back(x_point);
...
bool prove_invariant_set(
  network_handler system_network,
  unsigned int no_of_steps,
  vector< unsigned int > important_outputs,
  vector< datatype > scaling_factor,
  vector< datatype > offset_already,
  set_info target_set,
  vector< vector< double > > total_directions,
  vector< vector< double > > & final_set_biases
)
{

  final_set_biases.clear();
  ...
  simulate_accelerated(system_network, no_of_steps, important_outputs, scaling_factor,
    offset_already, current_set.region_constr, biases_next);
  ...
  next_set.time_stamp = current_set.time_stamp + no_of_steps;

  if(find_if_subset(next_set.region_constr , target_set.region_constr) )
  {
      final_set_biases = biases_next;
      return true;
  }

  cout << "Next set is : " << endl;
  print_biases(biases_next);
  return false;
}
